...
// ---- DejaView ES-module API (everything above is unmodified jpeg-js) ----------

/** Decode a baseline/progressive JPEG to RGBA. Pure-JS; runs in Node and browsers. */
...
/**
 * Hybrid JPEG -> pixmap: prefer the browser's native codec (fast, handles every
 * variant) when available, otherwise fall back to the pure-JS decoder above.
 * Returns an object shaped like the IW44 pixmap (getPixmap()).
 */
export async function decodeJpegPixmap(bytes) {
  if (typeof createImageBitmap === 'function' && typeof OffscreenCanvas === 'function') {
    try {
      ...
      return { getPixmap: () => pm };
    } catch (e) { /* fall through to the pure-JS decoder */ }
  }
  const pm = decodeJpeg(bytes);
  return { getPixmap: () => pm };
}
